<script>
    $(document).ready(function() {
        ...
        var table = $('#datatable').DataTable({
            ...
            ajax: {
                url: 'banner/data', // URL file untuk proses select datanya
                type: 'POST',
                data: {
            ...
            columnDefs: [{
                    ...
                    render: function(data, type, row, meta) {
                        return '<img src="<?= base_url('assets/images/banner/'); ?>' + data + '" alt="Gambar Kategori" class="img-thumbnail" style="max-width: 100px;">';
                    }
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        return '<a href="/admin/banner/status/' + row[5] + '" class="badge bg-success" id="status">' + status + '</a>';
                    }
                },
                {
                    ...
                    orderable: false, // Nonaktifkan sorting untuk kolom aksi
                    render: function(data, type, row, meta) {
                        return '<a href="/admin/banner/edit/' + data + '" class="btn btn-sm btn-primary" id="btn-edit"><i class="bx bx-pencil me-0"></i></a>' +
                            '<a href="/admin/banner/delete/' + data + '" class="ms-2 btn btn-sm btn-danger"><i class="bx bx-trash me-0"></i></a>';
                    }
                },
            ],
            pageLength: 25,
        ...
        $('#datatable').on('click', 'tbody tr td a#btn-edit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 200) {
                        $('#nama_banner').val(response.data.nama_banner);
                        $('#deskripsi_banner').val(response.data.deskripsi_banner);
                        $('#preview-gambar').attr('src', response.data.gambar_banner ? '<?= base_url('assets/images/banner/'); ?>' + response.data.gambar_banner : '').show();
                        $('#form-data').attr('action', '<?= base_url('admin/banner/edit/'); ?>' + response.data.id_banner);
        ...
        // hendle status
        table.on('click', 'tbody tr td a#status', function(e) {
            e.preventDefault();
            var url = $(this).attr('href');
            $.ajax({
                url: url,
                ...
                success: function(response) {
                    if (response.status === 200) {
                        Toast.fire({
                            timer: 2000,
                            icon: 'success',
                            title: response.message || 'Data berhasil diubah.'
                        });
                        table.ajax.reload(null, false); // Reload data tanpa reset pagination
                    } else {
                        Toast.fire({
                ...
                error: function() {
                    Toast.fire({
                        timer: 2000,
                        icon: 'error',
                        title: 'Terjadi kesalahan saat megirim data.'
                    });
                }
            });
