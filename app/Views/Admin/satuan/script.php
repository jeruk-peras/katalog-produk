<script>
    $(document).ready(function() {
        ...
        var table = $('#datatable').DataTable({
            ...
            ajax: {
                url: 'satuan/data', // URL file untuk proses select datanya
                type: 'POST',
                data: {
            ...
            columnDefs: [
                {
                    ...
                    render: function(data, type, row, meta) {
                        return '<a href="/admin/satuan/edit/' + data + '" class="btn btn-sm btn-primary"><i class="bx bx-pencil me-0"></i></a>' +
                            '<a href="/admin/satuan/delete/' + data + '" class="ms-2 btn btn-sm btn-danger"><i class="bx bx-trash me-0"></i></a>';
                    }
                },
            ],
            pageLength: 25,
        ...
        $('#modal-form').on('hidden.bs.modal', function() {
            $('#form-satuan').attr('action', '');
            $('#form-satuan')[0].reset(); // Reset form saat modal ditutup
            $('#form-satuan .form-control').removeClass('is-invalid');
        ...
        $('#form-satuan').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 200) {
                        ...
                        $('#form-satuan')[0].reset(); // Reset form setelah submit
                        $('#form-satuan .form-control').removeClass('is-invalid');
                        $('.invalid-feedback').text('');
                    } else {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 200) {
                        ...
                        $('#form-satuan').attr('action', '<?= base_url('admin/satuan/edit/'); ?>' + response.data.id_satuan);
