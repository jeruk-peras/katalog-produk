<script>
    $(document).ready(function() {
        ...
        var table = $('#datatable').DataTable({
            ...
            ajax: {
                url: 'patner/data', // URL file untuk proses select datanya
                type: 'POST',
                data: {
            ...
            columnDefs: [{
                    ...
                    render: function(data, type, row, meta) {
                        return `<img src="<?= base_url('assets/images/patner/'); ?>${data}" alt="${row[1]}" class="img-fluid">`; // Tampilkan slug
                    }
                },
                {
                    ...
                    orderable: false, // Nonaktifkan sorting untuk kolom aksi
                    render: function(data, type, row, meta) {
                        return '<a href="/admin/patner/edit/' + data + '" class="btn btn-sm btn-primary btn-edit"><i class="bx bx-pencil me-0"></i></a>' +
                            '<a href="/admin/patner/delete/' + data + '" class="ms-2 btn btn-sm btn-danger"><i class="bx bx-trash me-0"></i></a>';
                    }
                },
            ],
            pageLength: 25,
        ...
        $('#datatable').on('click', 'tbody tr td a.btn-edit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 200) {
                        $('#nama_patner').val(response.data.nama_patner);
                        $('#preview-gambar').attr('src', response.data.logo_patner ? '<?= base_url('assets/images/patner/'); ?>' + response.data.logo_patner : '').show();
                        $('#form-data').attr('action', '<?= base_url('admin/patner/edit/'); ?>' + response.data.id_patner);
        ...
        table.on('click', 'tbody tr td a.btn-danger', function(e) {
         ...
         // preview gambar
        $('#logo_patner').on('change', function() {
